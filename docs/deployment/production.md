...
| RabbitMQ | Native AMQP DNS/address pinning, bounded connection setup, publisher confirms, manual settlement, pool cleanup, and partial-setup/shutdown failure tests | Target TLS/CA, credentials/vhost permissions, firewall/DNS policy, acknowledgement timeout, quorum/durability policy, broker failover, load, and redelivery behavior |
