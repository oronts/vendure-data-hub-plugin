...
const LOG_CONTEXT = 'ShopifyExportGenerator';
    ...
    async generate(context: FeedGeneratorContext): Promise<CustomFeedResult> {
        ...
        for (const [productId, variants] of productGroups) {
            try {
                ...
                if (!product) {
                    console.warn(`[${LOG_CONTEXT}] Skipping variants for product ${productId}: product is undefined`);
                    continue;
                }
                ...
                for (const variant of variants) {
                    try {
                        const row = buildShopifyRow(variant, product, baseUrl, isFirstVariant, brandFacet, typeFacet, tags);
                        rows.push(row);
                ...
                console.warn(`[${LOG_CONTEXT}] Failed to process product group ${productId}: ${error}`);
            }
        }
...
interface ProductData {
    ...
    facetValues?: FacetValueWithFacet[];
}

function buildShopifyRow(
    variant: VariantWithCustomFields,
    product: ProductData,
    baseUrl: string,
    isFirstVariant: boolean,
    brandFacet: FacetValueWithFacet | undefined,
    typeFacet: FacetValueWithFacet | undefined,
    tags: string,
): Partial<ShopifyProductRow> {
