...
const rawWebhookJsonBodyParser = json({
    // HMAC verification must cover the bytes received from the sender, not an
    // inflated representation produced by request middleware.
    inflate: false,
...
interface JsonParserError {
    status: HttpStatus;
    error: string;
}

function resolveJsonParserError(
    error: unknown,
    bodyLabel: string,
...
export function resolveUploadJsonParserError(
    ...
    return resolveJsonParserError(error, 'JSON upload');
}

export function resolveWebhookJsonParserError(
    error: unknown,
): JsonParserError | undefined {
    return resolveJsonParserError(error, 'Webhook JSON');
}
...
export const dataHubJsonBodyParser: RequestHandler = (request, response, next) => {
    if (isWebhookRequest(request)) {
        rawWebhookJsonBodyParser(request, response, error => {
            if (!error) {
                next();
                return;
            }
            const inputError = resolveWebhookJsonParserError(error);
            if (!inputError) {
                next(error);
                return;
            }
