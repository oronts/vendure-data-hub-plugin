...
function validateFtpConfig(config: FtpExtractorConfig): void {
    normalizeRemoteHostname(config.host);
    for (const field of RAW_FTP_CREDENTIAL_FIELDS) {
        if (Object.prototype.hasOwnProperty.call(config, field)) {
            throw new Error(`FTP/SFTP field "${field}" must use a Secret Code reference`);
        }
    }
}
...
export async function buildSftpConnectionOptions(
    ...
    if (config.hostKeyFingerprintSecretCode) {
        options.hostKeyFingerprint = await context.secrets.get(config.hostKeyFingerprintSecretCode);
    }
...
export async function createFtpClient(
    ...
    const remote = await resolveSafeRemoteAddress(options.host);
    const client = new BasicFtpClient(options.timeout);
...
export async function createSftpClient(
    ...
    const hostVerifier = createSftpHostVerifier(options.hostKeyFingerprint);
    const { socket } = await connectPinnedRemoteSocket(
        options.host,
        options.port,
        options.timeout ?? FTP_DEFAULTS.timeoutMs,
    );
