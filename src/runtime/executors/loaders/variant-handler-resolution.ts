...
export async function resolveAllVariantOptionIds(
    ...
    const collected: ID[] = [];
    await collectCreatedOptionIds(services, ctx, record, productId, config, optionCache, logger, collected);
    await collectResolvedOptionIds(services, ctx, record, config, optionCache, logger, collected);
...
async function collectCreatedOptionIds(
    ...
    const optionsMap = getObjectValue(record, config.optionGroupsField) as Record<string, string> | undefined;
...
async function collectResolvedOptionIds(
    ...
    const codes = record[config.optionCodesField];
...
function collectDirectOptionIds(
    ...
    const directIds = record[config.optionIdsField];
