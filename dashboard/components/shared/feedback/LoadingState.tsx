...
export function LoadingState({
    ...
    const { t } = useLingui();
    const effectiveMessage = message ?? t`Loading...`;
    if (type === LOADING_STATE_TYPE.SPINNER) {
        return (
            ...
            >
                <Loader2
                    className={`${ICON_SIZES.XL} animate-spin text-muted-foreground`}
                    aria-hidden="true"
                />
                {effectiveMessage && <span className="ml-3 text-muted-foreground">{effectiveMessage}</span>}
            </div>
        );
    }
    ...
    if (type === LOADING_STATE_TYPE.TABLE) {
        ...
        return (
            <div
                ...
                aria-busy="true"
            >
                <span className="sr-only">{effectiveMessage}</span>
                ...
                {Array.from({ length: rows }).map((_, rowIndex) => (
                    <div key={`row-${rowIndex}`} className="flex gap-4 py-2" aria-hidden="true">
                        {[0, 1, 2, 3].map((colIndex) => (
                            <div
                                key={`cell-${colIndex}`}
                                className="h-4 bg-muted rounded animate-pulse"
        ...
        return (
            ...
            >
                ...
                {Array.from({ length: rows }).map((_, fieldIndex) => (
                    <div key={`field-${fieldIndex}`} className="space-y-2" aria-hidden="true">
        ...
        return (
            ...
            >
                ...
                {Array.from({ length: rows }).map((_, cardIndex) => (
                    <Card key={`card-${cardIndex}`} aria-hidden="true">
        ...
        return (
            ...
            >
                ...
                {Array.from({ length: rows }).map((_, itemIndex) => (
                    <div key={`item-${itemIndex}`} className="flex items-center gap-3 p-2" aria-hidden="true">
