    ...
    return (
        <div className={`bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800 rounded-md ${compact ? 'p-2.5' : 'p-3'}`}>
            <div className="flex items-center gap-2 text-amber-800 dark:text-amber-400">
                ...
                <span className="text-sm font-medium">Select {'aeiouAEIOU'.includes(adapterTypeLabel[0]) ? 'an' : 'a'} {adapterTypeLabel}</span>
            </div>
            <p className={`text-xs text-amber-700 dark:text-amber-500 mt-1 ${compact ? 'ml-6' : ''}`}>
                This step requires {'aeiouAEIOU'.includes(adapterTypeLabel[0]) ? 'an' : 'a'} {adapterTypeLabel} to be configured.
            </p>
        </div>
    );
});
